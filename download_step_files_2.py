...
SAVE_DIR = "E:/step_files"  # Directory to save downloaded files
...
def download_file(s3, row):
    ...
    try:
        ...
        s3.download_file(
            bucket_name,
            file_key,
            f"{SAVE_DIR}/{item_id}_{file_name}"
        )
    except Exception as e:
        print(f"Failed: {row.download_file_url} — {e}")
...
with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
    for _, row in data_na.iterrows():
        futures.append(executor.submit(download_file_with_s3, row))
...
for future in as_completed(futures):
    try:
        ...
        print(f"Unexpected thread exception: {e}")
